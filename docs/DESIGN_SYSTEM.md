...
### 액센트 (모노톤)
운영 결정: **컬러 액센트 없는 모노크롬.** 하이라이트·활성 상태는 별도 포인트 컬러 대신
풀-오패시티 화이트(`paper` `#fafafa`)로 표현한다. 본문은 `paper/70` 이하의 흐린 화이트라
액센트 요소가 100% 밝기로 떠서 자연스럽게 구분된다.

예: 활성 칩은 `border-paper/70` + `bg-paper/[0.06]`, 페스티벌 뱃지는 채움 대신 아웃라인,
강조 점·라벨은 `text-paper` / `bg-paper`.
...
- ❌ 글로우·네온 효과
- ❌ 좌측 보더 액센트 (`border-l-4`) — 빈티지 블로그 UI 트로프
- ❌ 이모지
