...
## 파싱 규칙
- 챕터 우선, 없으면 설명란의 `MM:SS 라벨` 줄을 파싱. 둘 다 없으면 실패(수동 보강 필요).
- 비-곡 섹션 스킵: `입장/Intro/Outro/오프닝/엔딩/ending/ment/멘트/talk/MC/인사/interlude` 등.
- `앙코르/encore/앵콜` 단독 챕터 = 구간 마커 → 이후 곡들 `isEncore=true`.
- `1. Sister` 의 "1. " 인덱스만 제거하고 곡명은 그대로(`Machineboy空`, `T + Tik Tak Tok` 등 유지).
